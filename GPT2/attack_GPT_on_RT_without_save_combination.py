...
result_file_save_path = os.path.join(current_dir, 'results/GPT_RT_1.csv')
final_result_file_save_path = os.path.join(current_dir, 'results/GPT_RT_1_final.csv')
...
with open(result_file_save_path, 'w', newline='', encoding='utf-8') as result_file:
    ...
    for randSeed in cycle_number:
        ...
        # set the dataset
        train_dataset = attack_auxiliary_function.preprocess_tsv_sst(data_name, tokenizer=GPT2_tokenizer)
            ...
            for index_vocab_batches, vocab_size_batch_temp in enumerate(vocab_batches):
                ...
                with torch.no_grad():  # Disable gradient calculation to save memory
                    result_embedding = model.forward_partial_0(input_ids=input_ids_group,
                                                               position_ids=position_ids_group)
                ...
                for temp_accumulated_list in accumulated_list:
                    ...
                    if is_subset:
                        possible_sentence_here = attack_auxiliary_function.GPT_process_combinations_without_modeltest_csv_opt(
                            ...
                            device)
                        # print("possible_sentence_here:", possible_sentence_here)
