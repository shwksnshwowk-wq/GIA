...
with open(result_file_save_path, 'w', newline='', encoding='utf-8') as result_file:
    ...
    for randSeed in cycle_number:
        ...
        for num_batch_size, batch_size_temp in enumerate(batch_size):
            ...
            # 利用index_effect来重构每个index对应的token
            decode_token = []
            for temp_token in index_effect:
                temp_token = qwen_tokenizer.decode([temp_token], clean_up_tokenization_spaces=True)
            ...
            writer.writerow([decode_token, random_batch_target["sentence"]])
