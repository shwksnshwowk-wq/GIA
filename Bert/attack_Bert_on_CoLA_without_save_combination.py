...
res_p = 'results/ablation_Bert_cola_' + str(noise_multiplier) + '_' + str(reduction_factor) + '.csv'
cycle_number = range(1, 10, 1)  # the number of experiments
# the pos of results
current_dir = os.path.dirname(os.path.abspath(__file__))
result_file_save_path = os.path.join(current_dir, res_p)
...
with open(result_file_save_path, 'w', newline='', encoding='utf-8') as result_file:
    ...
    for randSeed in cycle_number:
        ...
        for num_batch_size, batch_size_temp in enumerate(batch_size):
            ...
            # print("Token_index_all：", Token_index_all)
            ...
            rouge_2_score = []
            if true_sentence != []:
                true_sentence = bert_tokenizer(true_sentence, truncation=True, padding=True, return_tensors='pt').to(
                    device)
...
with open(final_result_file_save_path, 'w', newline='') as final_file:
    ...
    for num_batch_size, batch_size_temp in enumerate(batch_size):
        ...
        writer.writerow(
            [batch_size_temp, ave_rec_num_final, ave_com_over_final, ave_ave_rouge_1_final, ave_ave_rouge_2_final,
             token_num_final, min_len_final])
