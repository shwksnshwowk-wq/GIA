...
batch_size = range(1, 513, 1)        # the batch_size settings
reduction_factor = 2             # reduction_factor setting
cycle_number = range(0, 3, 1)    # the number of experiments
...
if not os.path.exists(temp_file):
    with open(temp_file, mode='w', newline='', encoding='utf-8') as f:
        ...
        for index_vocab_batches, vocab_batch in enumerate(vocab_batches):
            ...
            with torch.no_grad():
                layer_output, layer_output_embeddings, layer_output_1 = model(input_ids=vocab_size_batch_temp, attention_mask=attention_mask)
...
token_num_all = [[] for _ in range(len(batch_size))]
min_len_all = [[] for _ in range(len(batch_size))]
