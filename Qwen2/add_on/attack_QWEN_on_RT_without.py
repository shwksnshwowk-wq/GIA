...
data_name = 'E:/Dataset/rotten_tomatoes/rotten_tomatoes_train.csv'
num_labels = 20                  # number of classes
batch_size = range(1, 257, 1)        # the batch_size settings
reduction_factor = 2             # reduction_factor setting
cycle_number = range(0, 5, 1)    # the number of experiments

# the pos of results
current_dir = os.path.dirname(os.path.abspath(__file__))
result_file_save_path = os.path.join(current_dir, 'results/Qwen_RT_1.csv')
final_result_file_save_path = os.path.join(current_dir, 'results/Qwen_RT_1_final.csv')
...
with open(result_file_save_path, 'w', newline='', encoding='utf-8') as result_file:
    ...
    for randSeed in cycle_number:
        ...
        # load the datasets
        train_dataset = attack_auxiliary_function.preprocess_tsv_sst(data_name, tokenizer=qwen_tokenizer)
        ...
        for num_batch_size, batch_size_temp in enumerate(batch_size):
            ...
            writer.writerow([randSeed, batch_size_temp, len(index_effect), len(true_token)])
            result_file.flush()
