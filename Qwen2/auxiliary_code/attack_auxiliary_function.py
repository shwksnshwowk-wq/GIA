...
def can_be_expressed_two(vectors, target, device):
    ...
    isclose_tensor = torch.isclose(reconstructed_vector_in.t(), target_flattened, atol=1e-4, rtol=1e-4)
    ...
    isclose_tensor = torch.isclose(reconstructed_vector_flattened.t(), target_flattened, atol=1e-4, rtol=1e-4)
    is_close_in = torch.all(isclose_tensor, dim=1)
...
def process_combinations_without_modeltest_csv_opt(combination_result_temp, bert_tokenizer, model, model_word_test,
    ...
    for batch in batches:
        ...
        layer_output_1 = model.forward_partial_1(input_ids=input_ids, position_ids=position_ids,
                                                 ...
                                                 head_mask=head_mask)
        is_close_in_rows, is_close_in_cols, coefficients, reconstructed_vector = can_be_expressed(
...
def GPT_process_combinations_without_modeltest_csv_opt(combination_result_temp, GPT2_tokenizer, model, model_word_test,
    ...
    is_close_in_rows, is_close_in_cols, coefficients, reconstructed_vector = can_be_expressed_one(weight_divided_bias_0,
                                                                                              layer_output_1, device)
    if is_close_in_rows.numel() > 0:
        text_combination_result = [
            GPT2_tokenizer.decode(ids, clean_up_tokenization_spaces=True, skip_special_tokens=True)
            for ids in combination_result_temp]
...
# code for GPT2
def Qwen_process_combinations_without_modeltest_csv_opt(combination_result_temp, tokenizer, model, weight_divided_bias_0, device):
    ...
    batch_size, max_seq_length = input_ids.size()
    attention_mask = torch.tensor([[1.0] * max_seq_length]).to(device)
    layer_output, layer_output_embeddings, layer_output_1 = model(input_ids=input_ids, attention_mask=attention_mask)
    is_close_in_rows, is_close_in_cols, coefficients, reconstructed_vector = can_be_expressed_two(weight_divided_bias_0, layer_output_1, device)
    if is_close_in_rows.numel() > 0:
        combination_result_temp = torch.stack(combination_result_temp, dim=0).unsqueeze(0)
        possible_sentence = [tokenizer.decode(ids.cpu(), clean_up_tokenization_spaces=True, skip_special_tokens=True) for ids in combination_result_temp]
